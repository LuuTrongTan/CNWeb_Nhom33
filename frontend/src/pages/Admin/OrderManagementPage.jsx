...
const OrderManagementPage = () => {
  ...
  const fetchOrderStats = async () => {
    try {
      ...
      if (
        ...
        allOrdersResponse.data.results &&
        Array.isArray(allOrdersResponse.data.results)
      ) {
        const allOrders = allOrdersResponse.data.results;
  ...
  const handleStatusChange = async (order, newStatus) => {
    try {
      ...
      displayToast(
        `Đơn hàng ${
          order.orderNumber || formatOrderNumber(order)
  ...
  const handlePaymentStatusChange = async (order) => {
    try {
      ...
      displayToast(
        `Lỗi khi cập nhật trạng thái thanh toán: ${err.message}`,
        "error"
      );
    }
  };
  ...
  const handleSearch = (e) => {
    ...
    if (searchTerm.trim()) {
      displayToast(
        `Đang tìm kiếm đơn hàng với từ khóa "${searchTerm}"`,
        "info"
      );
    } else {
          ...
          <div className="header-actions">
            ...
            >
              <FontAwesomeIcon
                icon={faSync}
                className={
                  statsLoading || loading ? "button-icon spin" : "button-icon"
                }
              />
            ...
            {orders.length === 0 ? (
              ...
              orders.map((order) => (
                <tr
                  ...
                  className={
                    hoveredRowId === (order._id || order.id)
                      ? "highlight-row"
                      : ""
                  }
