    ...
    const fetchCart = async () => {
      if (isAuthenticated) {
        try {
            ...
            if (localCart) {
              try {
                ...
                if (
                  ...
                  const syncResult = await apiClient.post("/cart/sync", {
                  ...
                  if (syncResult.data.cart) {
                    ...
                    setSelectedItems(
                      updatedItems.map((item) => item.cartItemId)
                    );
                  }
                }
              } catch (e) {
          ...
          const errorMessage =
            err.response?.data?.message || "Không thể lấy giỏ hàng từ server";
          ...
          try {
            ...
            const cartWithIds = parsedCart.map((item) => {
              if (!item.cartItemId) {
                return {
                  ...item,
                  cartItemId: `${item.id}-${item.selectedSize || "no-size"}-${
                    item.selectedColor || "no-color"
                  }-${Date.now()}`,
                };
              }
        ...
        if (existing) {
          ...
          return [
            ...prev,
            {
              ...
              cartItemId: `${productToAdd.id}-${
                productToAdd.selectedSize || "no-size"
              }-${productToAdd.selectedColor || "no-color"}-${Date.now()}`,
            },
          ];
        }
      });
      ...
      try {
        ...
        if (response.data && response.data.items) {
          ...
          setSelectedItems((prev) =>
            prev.filter((id) =>
              formattedItems.some((item) => item.cartItemId === id)
            )
          );
        ...
        const errorMessage =
          err.response?.data?.message || "Không thể xóa sản phẩm khỏi giỏ hàng";
        showError(errorMessage);
  ...
  const updateQuantity = async (itemId, quantity) => {
    ...
    if (isAuthenticated) {
      ...
      try {
        ...
        const errorMessage =
          err.response?.data?.message || "Không thể cập nhật số lượng sản phẩm";
        showError(errorMessage);
      } finally {
        setLoading(false);
      }
  ...
  const updateCartItem = async (itemId, updateData) => {
    ...
    if (isAuthenticated) {
      ...
      try {
        ...
        const newSize =
          selectedSize !== undefined ? selectedSize : currentItem.selectedSize;
        const newColor =
        ...
        if (duplicateItem) {
          ...
          const totalQuantity =
            (quantity || currentItem.quantity) + duplicateItem.quantity;
          ...
          if (selectedSize !== undefined)
            updatePayload.selectedSize = selectedSize;
          if (selectedColor !== undefined)
            updatePayload.selectedColor = selectedColor;
      ...
      setCart((prev) => {
        ...
        if (duplicateItem) {
          ...
          setSelectedItems((prev) => {
            const newSelected = [
              ...prev.filter(
                (id) => id !== itemId && id !== duplicateItem.cartItemId
              ),
              mergedItem.cartItemId,
            ];
            return newSelected;
          });
          ...
          if (
            ...
            updatedItem.cartItemId = `${currentItem.id}-${
              newSize || "no-size"
            }-${newColor || "no-color"}-${Date.now()}`;
          ...
          const result = prev.map((item) =>
            item.cartItemId === itemId ? updatedItem : item
          );
          localStorage.setItem("cart", JSON.stringify(result));
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        const remainingItems = cart.filter(
          (item) => !selectedItems.includes(item.cartItemId || item.id)
        );
        setCart(remainingItems);
        setSelectedItems(
          remainingItems.map((item) => item.cartItemId || item.id)
        );
